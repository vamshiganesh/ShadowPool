...
export function BeamWrapper({ children, className }: BeamWrapperProps) {
  return (
    <div className={cn('group relative inline-flex rounded-pill p-px', className)}>
      <div
        className="absolute inset-0 rounded-pill opacity-50 transition-opacity duration-300 group-hover:opacity-80 motion-reduce:animate-none animate-beam-rotate"
        style={{
          background:
...
export function BeamButton({
  ...
  return (
    <BeamWrapper>
      <button
        className={cn(beamButtonClasses, sizeClasses[size], className)}
        disabled={disabled}
        {...props}
      >
        {children}
...
export function BeamLink({ children, className, size = 'md', ...props }: BeamLinkProps) {
  return (
    <BeamWrapper>
      <Link className={cn(beamButtonClasses, sizeClasses[size], className)} {...props}>
        {children}
      </Link>
    </BeamWrapper>
  )
